...
from Installation.nnAudio.librosa_functions import *
from Gon.utils import *
...
class CQT1992v2(torch.nn.Module):
    ...
    def __init__(self, sr=22050, hop_length=512, fmin=32.70, fmax=None, n_bins=84,
        ...
        start = time()
        cqt_kernels, self.kernel_width, lengths, freqs = create_cqt_kernels(Q,
                                                                     sr,
                                                                     fmin,
                                                                     n_bins,
                                                                     bins_per_octave,
                                                                     ...
                                                                     window,
                                                                     fmax)
        self.register_buffer('lenghts', lengths)
        self.freqs = freqs
    ...
    def forward(self,x, output_format=None):
        ...
        # CQT
        CQT_real = conv1d(x, self.cqt_kernels_real, stride=self.hop_length) * 2
        CQT_imag = -conv1d(x, self.cqt_kernels_imag, stride=self.hop_length) * 2
